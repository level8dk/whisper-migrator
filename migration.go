...
func NewConfig(wspFile string) *TagConfig {
	...
	newTagConfig.Pattern = strings.TrimRight(input, "\r\n")
	if len(newTagConfig.Pattern) == 0 {
		return nil
	}

	fmt.Println("\nPlease enter measurement e.g. #TEXT3, \n\t#TEXT3 will be replaced with actual value->")

	input, _ = reader.ReadString('\n')
	newTagConfig.Measurement = strings.TrimRight(input, "\r\n")
	if len(newTagConfig.Measurement) == 0 {
		return nil
	}

	fmt.Println("\nPlease enter tags e.g. host=#TEXT1 loc=#TEXT2 \n\t" +
		"host and loc are the tag keys and #TEXT1, #TEXT2 will be replaced \n\t" +
		"actual tag values->")

	input, _ = reader.ReadString('\n')
	tagDataStr := strings.TrimRight(input, "\r\n")
	...
	fmt.Println("\nPlease enter Field e.g. value->")
	input, _ = reader.ReadString('\n')
	newTagConfig.Field = strings.TrimRight(input, "\r\n")
...
func (migrationData *MigrationData) CreateShards() error {
	...
	fmt.Println("Finish creating Shards")
	return nil
}
...
func (migrationData *MigrationData) GetTSMFileName(shard ShardInfo) string {
	if migrationData.retentionPolicy == "" {
		// Default retention policy generated by InfluxDB if no retentions is defined for database
		migrationData.retentionPolicy = "autogen"
	}

	filename := "000000001-000000002.tsm" // TODO:..

	filePath := path.Join(migrationData.influxDataDir, migrationData.dbName, migrationData.retentionPolicy, shard.id.String(), filename)
	return filePath
}
...
func (migrationData *MigrationData) GetMTF(wspFilename string) *MTF {
	...
	//fmt.Println("wspFilename: ", wspFilename)
	...
	for _, tagConfig = range migrationData.tagConfigs {
		...
		regPat := ".*." + reg.ReplaceAllString(tagConfig.Pattern, ".*")

		//fmt.Println("regPat: ", regPat)

		patternStr = strings.Split(tagConfig.Pattern, ".")
		re := regexp.MustCompile(regPat)

		match := re.Match([]byte(wspFilename))
	...
	for i := 1; i < len(patternStr)-1; i++ {
		...
		//fmt.Println("patternTagValue: ", patternTagValue)
		...
		for j, tagkeyvalue := range tagConfig.Tags {
			if strings.Trim(tagkeyvalue.Tagvalue, "#") == patternTagValue {
				...
				mtf.Tags[j].Tagvalue = remArr[i]
			}
		}
		// Or our Measuremnt
		if strings.Trim(tagConfig.Measurement, "#") == patternTagValue {
			mtf.Measurement = remArr[i]
		}

	}

	for j, tagkeyvalue := range tagConfig.Tags {
		if !strings.HasPrefix(tagkeyvalue.Tagvalue, "#") {
			mtf.Tags[j].Tagkey = tagkeyvalue.Tagkey
			mtf.Tags[j].Tagvalue = tagkeyvalue.Tagvalue
		}
	}

	// If Measurement is not defined, assign the last string as measurement
...
func (migrationData *MigrationData) WriteUsingV2() {
	...
	for _, wspFile := range migrationData.wspFiles {
		...
		fmt.Println("Data point # ", len(wspPoints))
		if err != nil {
			log.Fatal(err)
		}
